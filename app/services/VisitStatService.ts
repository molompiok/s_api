...
export class VisitStatsService {
    ...
    `
        ...
        if (include.landing_page) {
            const landing_pageStatsSQL = generateStatSQL(groupFormat, 'landing_page', whereClause)
            const landing_pageStats = await db.rawQuery(landing_pageStatsSQL, bindings)
            landing_pageMap = buildFieldStatsMap(landing_pageStats.rows, 'landing_page')
        }
        ...
        const final: VisitStatsResult[] = mainStats.rows.map((row: any) => {
            ...
            if (include.landing_page && landing_pageMap[row.date]) {
                res.landing_page = landing_pageMap[row.date];
            }
