...
export default class CategoriesController {
    ...
    private getCategoriesSchema = vine.compile(
      vine.object({
        ...
        // user_id: vine.string().uuid().optional(), // Remplacé par auth/bouncer
    ...
    private getFiltersSchema = vine.compile(
     ...
     private createCategorySchema = vine.compile(
         vine.object({
             ...
             // 'view' et 'icon' gérés par createFiles
         })
     );
     ...
     private updateCategorySchema = vine.compile(
        vine.object({
            ...
            view: vine.any().optional(), // ✅ Utiliser any pour Vine, sera normalisé
            icon: vine.any().optional(), // ✅ Utiliser any pour Vine, sera normalisé
        })
     );

      private deleteCategoryParamsSchema = vine.compile(
          vine.object({
    ...
    async get_categories({ response, request }: HttpContext) { // Retiré auth
        ...
        try {
            ...
            if (payload.category_id) {
                 const category = await query.where('id', payload.category_id).first(); // Utiliser .first()
                 if (!category) {
                      // 🌍 i18n
                      return response.notFound({ message: t('category.notFound') }); // Nouvelle clé
                 }
                 // Ajouter le compte produit si demandé (si la relation withCount a fonctionné)
                 // category.product_count = category.$extras.products_count ?? 0;
                 return response.ok(category);
            }
             // 🔍 GET par Slug
            if (payload.slug) {
                 const category = await query.where('slug', payload.slug).first(); // Utiliser .first()
                 if (!category) {
                      // 🌍 i18n
            ...
            //      ...cat.serialize(),
            //      product_count: cat.$extras.products_count ?? 0
            // }));
    ...
    async create_category({ request, response, auth, bouncer }: HttpContext) {
        ...
        try {
            ...
            payload = await this.createCategorySchema.validate(request.all());
    ...
    async update_category({ request, response, auth, bouncer }: HttpContext) {
         // 🔐 Authentification
         await auth.authenticate();
         // 🛡️ Permissions
         try {
        ...
        try {
            ...
            const category_id = payload.category_id; // ID validé
    ...
    async delete_category({ params, response, auth, bouncer }: HttpContext) {
        ...
        let payload: Infer<typeof this.deleteCategoryParamsSchema>;
        try {
            // ✅ Validation Vine pour Params
            payload = await this.deleteCategoryParamsSchema.validate(params);
        } catch (error) {
             if (error.code === 'E_VALIDATION_ERROR') {
                 // 🌍 i18n
                 return response.badRequest({ message: t('validationFailed'), errors: error.messages });
        ...
        try {
             ...
             const productsToUpdate = await db.from(Product.table)
