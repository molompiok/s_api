...
export default class ProductsController {
  ...
  private deleteProductParamsSchema = vine.compile(
    vine.object({
      id: vine.string().uuid(),
    })
  );
  ...
  async create_product({ request, response, auth, bouncer }: HttpContext) {
    ...
    try {
      // ✅ Validation Vine
      const payload = await request.validateUsing(this.createProductSchema)
      ...
      const feature = await Feature.create({
        id: feature_id,
        product_id,
        // 🌍 i18n (possible pour le nom par défaut, mais complexe)
 ...
 private applySearch(query: any, search?: string) {
    ...
    try {
      ...
      if (payload.slug_cat) {
        ...
        category = await Categorie.query()
          ...
          // 🔍 .first() -> remplacé par firstOrFail pour correspondre à la logique initiale
          .firstOrFail()
      }

      if (payload.slug_product) query = query.where('slug', payload.slug_product)
      // 🔍 Gestion de product_id : la logique veut lister, donc pas de .first() ici.
      if (payload.product_id) query = query.where('id', payload.product_id)
      ...
      if (payload.order_by) query = applyOrderBy(query, payload.order_by, Product.table)
  ...
  async update_product({ request, response, auth, bouncer }: HttpContext) {
     // 🔐 Authentification
    await auth.authenticate();
    // 🛡️ Permissions
    try {
  ...
  async delete_product({ params, response, auth, bouncer }: HttpContext) {
    ...
    let payload: Infer<typeof this.deleteProductParamsSchema>;
    try {
        // ✅ Validation Vine pour Params
        payload = await this.deleteProductParamsSchema.validate(params)
    } catch (error) {
       if (error.code === 'E_VALIDATION_ERROR') {
         // 🌍 i18n
         return response.badRequest({ message: t('validationFailed'), errors: error.messages })
